...
**Done**
- K2: `POST /api/kitchen/orders/{id}/accept` (kitchen, 204) — row-locked `Received→Preparing` via `transition()`, KITCHEN tracking note. Accept button on `Received` cards (409→refetch, 5xx→inline error).
- K3: `POST /api/kitchen/orders/{id}/mark-ready` (kitchen, 200 `{status}`) — courier request via `DeliveryPort`. Success→`ReadyForDispatch`+reference+TRANSPORT tracking; `DeliveryError`→`DispatchPending` (committed, not re-raised; 200 not 502)→makes admin A5 retry reachable. `MarkReadyOut.status` is a `Literal` (OpenAPI enum). Mark Ready button + queue-level deferred notice (survives the card leaving). `Ticket` extracted to `ticket.tsx` (queue-client <300 lines).
- Rebased onto `main` after #36 (A11+A12+A13) merged: re-registered `kitchen_actions_router` in `main.py`, regenerated `openapi.json` + `types.ts` on top of #36's contract, reconciled CONTRACTS/feature_list.

**Accepted deviations**
- `ReadyForDispatch` cards render no action button — Confirm Pickup is K4. Staff verbs role-guard only.

**Verified**
- See K2/K3 evidence in `feature_list.json`; gate re-run green post-rebase (backend pytest, contract parity, frontend tsc/eslint/vitest/build).

**Next**
- `K4` Confirm Pickup: Confirm Pickup action on `ReadyForDispatch` cards + `ReadyForDispatch→Delivering`.
...
**Verified**
- `./verify.sh` exit 0 at `29e7bf9`, `2026-06-11T17:40:17Z` (UTC; = 2026-06-12 00:40 +07 — matches the header's local date).
