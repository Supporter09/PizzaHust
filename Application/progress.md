...
## 2026-06-11 — ordering loop U5/U6/U7 + K1–K3/T1 + U11/U14 + A7 (closes Gate G2)

**Done**
- `app/services/orders.py`: shared `resolve_line` (item + combo, option snapshots),
  `place_order` (service-area + loyalty pipeline, `PIZZ-XXXXXX` codes, audit trail,
  redeem-deduct), `dispatch_order` (single delivery-handoff path), `accrue_loyalty_on_delivered`.
- `app/api/orders.py`: `POST /api/orders` (U6, guest+customer, optional-user dep),
  `GET /api/orders/track/{code}` (U7, public, 30/min/IP), `GET /api/orders/me` (U11).
- `app/api/kitchen.py`: `GET /api/kitchen/queue` (K1, kitchen|admin), `accept` (K2),
  `ready` (K3+T1 dispatch; provider fail → DispatchPending). State machine gains
  `ReadyForDispatch → DispatchPending`.
- `cart.py` refactored onto the shared resolver (combos now quotable — U5).
  `admin/orders.py` retry-dispatch refactored onto `dispatch_order`.
- Webhook (T2) now accrues loyalty on the Delivering→Delivered edge.
- `app/api/admin/reports.py`: `GET /api/admin/reports/sales(.csv)` (A7) — realized revenue,
  daily series, status counts, top items, AOV.
- Frontend: `CartProvider` (localStorage) + nav cart badge; menu detail "Add to cart"/"Buy now";
  `/cart` (U5), `/checkout` (U6, ward datalist from `/config/delivery`, live quote, redeem),
  `/track` (U7, OrderTimeline+items), `/orders` (U11 + reorder), `/kitchen` (K1–K3, polling),
  `/admin/reports` (A7, CSS bar charts + CSV). Role-aware nav links.
- Regenerated `openapi.json` + `frontend/lib/api/types.ts`; CONTRACTS.md "Ordering loop" section;
  `feature_list.json` U5–U14/A5–A7/K1–K3/T1/T2 → done (only A9/A10/U15/U16/K4 remain — stretch).

**Verified**
- Backend `pytest` **209 passed / 1 skipped** (was 175); new suites test_orders/test_kitchen
  (incl. full place→accept→ready→webhook-Delivered→accrual loop)/test_reports.
- Frontend `tsc` clean, `eslint` 0 errors (3 pre-existing `<img>` warnings), `vitest` 15 passed,
  `next build` green (all of /cart /checkout /track /orders /kitchen /admin/reports compile).
- Not run to completion: full `verify.sh` (needs Docker/MySQL + Playwright e2e for the loop).

**Next**
- Add Playwright e2e for the COD loop (re-enables the deferred happy-path) and a kitchen spec.
- Optional stretch: A9 multi-image, A10/U15 combo choice-slots, U16 line notes UI, K4 pickup.
