...
**Current state:** `A13` Admin Business Settings **done and verified** on branch `feat/admin-business-settings` at `013f644`, plus a folded-in follow-on: **loyalty accrual is now wired to order placement** (logged-in customers earn points at placement at the admin-configured rate; exact rate-safe reversal on cancel via the stored `orders.loyalty_points_earned` + migration `0016`; guests earn nothing; redemption stays stubbed for U14). Whole-branch review applied (SHIP, no Critical); accrual change reviewed APPROVED. The branch now stacks **A11** (Admin Menu Management Overhaul) + **A12** (per-category options) + **A13** (admin business settings) — none are on `main` yet. Business config (timezone, **per-ward delivery fees**, loyalty rules) is admin-editable from `/admin/settings`, replacing static constants; the UTC-vs-+07 date-window bug that had kept `admin-orders.spec.ts:28` red since A11 is **fixed**.

**Verification (the headline):** **`./verify.sh` EXIT 0 at `013f644` — the first clean exit-0 in the whole A11→A12→A13 lineage** (now includes the loyalty-accrual follow-on). Backend **393 passed/1 skipped** (ruff, mypy domain, lint-imports, alembic-drift all clean); frontend tsc/eslint clean, **80 vitest**, `next build` ok (`/admin/settings` route present); OpenAPI↔types parity clean; smoke 1 passed/1 skipped; Playwright **44 passed/4 skipped/0 FAILED**. (`cart.spec.ts:4` is a known combo parallel-load flake — passes on isolated rerun; verify exits 1 only if it flakes since this env lacks the chrome-debug retry helper.) The prior sole red gate `admin-orders.spec.ts:28` now **passes** (the timezone fix), and both new A13 e2e specs pass (settings page renders; editing a ward's fee is reflected in `GET /api/config/delivery`). `cart.spec.ts:4` (combo parallel-load flake) passed in the same run.
...
**Known follow-up:**
- **U14 — Redeem Points for Discount** is still stubbed: the quote/placement path hardcodes `current_points=0` (cart.py / orders.py), so redemption never applies and points are never deducted. Accrual (earn) is now wired; redemption (spend) is the remaining half. (The earlier A13 whitespace-ward and accrual-not-wired follow-ups are both now resolved.)
