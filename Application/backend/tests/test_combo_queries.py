...
def test_slot_availability_min_price_and_unavailable_modes():
    ...
    new_product(ok_cat, "Cola", base_price_vnd=15_000)
    new_product(ok_cat, "Juice", base_price_vnd=25_000)
    new_product(ok_cat, "Gone", base_price_vnd=5_000, is_active=False)
    inactive_cat = new_category("Hidden", is_active=False)
    new_product(inactive_cat, "X", base_price_vnd=10_000)
