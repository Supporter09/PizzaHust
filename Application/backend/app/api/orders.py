...
def place_order(
    ...
    # Loyalty accrual is recorded at placement but only credited when the order
    # reaches Delivered.
    earned_points = 0
    if session.user_id is not None:
    ...
    order = Order(
        ...
        loyalty_points_redeemed=quote.loyalty.redeemed,
    )
    db.add(order)
    db.flush()

    if session.user_id is not None and quote.loyalty.redeemed:
        assert user is not None
        user.current_points = max(0, user.current_points - quote.loyalty.redeemed)
