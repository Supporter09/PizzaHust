...
from app.seeds.catalog_images import (
    COMBO_IMAGES,
    PRODUCT_IMAGES,
    install_seed_blobs,
    seed_combo_image,
...
def _seed(db: Session, settings: Settings) -> None:
    ...
    # ── Catalog images: a real cover for every product (Margherita keeps a gallery) ──
    base = settings.image_base_url.rstrip("/")
    all_products = pizza_products + side_products + drink_products
    install_seed_blobs(
        settings,
        [f for p in all_products for f in PRODUCT_IMAGES.get(p.name, ())]
        + list(COMBO_IMAGES.values()),
    )
    for product in all_products:
        seed_product_images(db, base, product, PRODUCT_IMAGES.get(product.name, ()))
    ...
    for combo in (combo1, combo2, combo3):
        fname = COMBO_IMAGES.get(combo.name)
        if fname:
            seed_combo_image(db, base, combo, fname)
